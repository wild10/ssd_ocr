...
class SSD_finetune:
    ...
    def prepare_ds(self, dataset_folder,annotation_file=''):
        ...
        ds = DataGenerator()
        print('images_dir: ',dataset_folder,'labels_filename: ',annotation_file)
    ...
    def get_generator_2(self, batch_size, trans, anot_file, dataset_test,encoder='',returns={'processed_images','encoded_labels'},val=False):

        dataset = self.prepare_ds(dataset_test,anot_file)
    ...
    def training_plot(self, epoch, logs):
        ...
        if self.val_generator:
            ...
            for box in y_pred_decoded[0]:
                ...
                print("class id:", self.id2digit[class_id])
    ...
    def init_training(self):
        ...
        checkpoint = ModelCheckpoint(filepath='./logs/ssd_{:%Y%m%dT%H%M}'.format(now)+f'_{task}_classes_{self.n_classes}-'+'{epoch:02d}_loss-{loss:.4f}\_val_loss-{val_loss:.4f}.h5',monitor='val_loss', verbose=1,save_best_only=True, save_weights_only=True, mode='auto',period=1)
